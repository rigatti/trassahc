...
class ProductCore extends ObjectModel
{
	...
	*/
	...
	//[[ RM - START ]]
	static public function getWholesalePrice($id_product, $id_product_attribute = NULL) {
		/*
			$result = Db::getInstance()->getValue(
					'SELECT `wholesale_price`
					FROM `'._DB_PREFIX_.'product_attribute`
					WHERE `id_product` = '. $id_product . ' AND ' .
					(($id_product_attribute == NULL)?' `default_on` = 1':' `id_product_attribute` = ' . $id_product_attribute));
		*/
		$result = Db::getInstance()->getRow('
				SELECT p.`wholesale_price`,
				'.($id_product_attribute ? 'pa.`wholesale_price`' : 'IFNULL((SELECT pa.wholesale_price FROM `'._DB_PREFIX_.'product_attribute` pa WHERE id_product = '.intval($id_product).' AND default_on = 1), 0)').' AS attribute_wholesale_price
				FROM `'._DB_PREFIX_.'product` p
		...
		if ($result['attribute_wholesale_price']) {
			return floatval($result['attribute_wholesale_price']);
		} else {
			return floatval($result['wholesale_price']);
		}
	}
	//[[ RM - END ]]
	/**
	...
	*/
	...
	public function getAttributesGroups($id_lang)
	{
		if (!Combination::isFeatureActive())
			return array();
		// [[ RM - START END : Add pa.`wholesale_price` ]]
		$sql = 'SELECT ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, agl.`public_name` AS public_group_name,
				a.`id_attribute`, al.`name` AS attribute_name, a.`color` AS attribute_color, product_attribute_shop.`id_product_attribute`,
				IFNULL(stock.quantity, 0) as quantity, product_attribute_shop.`price`, product_attribute_shop.`ecotax`, product_attribute_shop.`weight`,
				product_attribute_shop.`default_on`, pa.`reference`, pa.`wholesale_price`, product_attribute_shop.`unit_price_impact`,
	...
	public static function getProductProperties($id_lang, $row, Context $context = null)
	{
		...
		//[[ RM - START ]]
		$row['wholesale_price'] = Product::getWholesalePrice($row['id_product']);
		//[[ RM - END ]]
