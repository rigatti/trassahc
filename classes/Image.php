...
class ImageCore extends ObjectModel
{
	 ...
	 * RM - START
	 */
	public static function updateDefaultCover($id_product, $id_image = NULL) {
		$sql = 'UPDATE '._DB_PREFIX_.'image SET cover = 0' ;
		...
		if ($id_image) {
			...
			$sql .= ' WHERE id_product = '.(int)$id_product;
			$imgIds = Db::getInstance()->executeS($sql);
			//echo '$imgIds:' . $imgIds;
			//echo 'print_r:';
			//print_r($imgIds);
			foreach ($imgIds as $imgId) {
				//echo 'print_r_foreach:';
				//print_r($imgId);
				$sql = 'UPDATE ' . _DB_PREFIX_ . 'image_shop SET cover = 0' ;
				$sql .= ' WHERE id_image = ' . (int)$imgId['id_image'];
				//echo 'A: ' . $sql;
				Db::getInstance()->execute($sql);
			}

			$sql = 'UPDATE ' . _DB_PREFIX_ . 'image SET cover = 1' ;
			$sql .= ' WHERE id_product = '.(int)$id_product . ' AND id_image = ' . (int)$id_image;
			Db::getInstance()->execute($sql);

			$sql = 'UPDATE ' . _DB_PREFIX_ . 'image_shop SET cover = 1' ;
