		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			*/
			...
			foreach ($packageByAddress as $id_package => $package)
			{
				...
				$order->total_paid_pro = (float)Tools::ps_round((float)$this->context->cart->getOrderTotalWholeSale(), 2);;
			...
			foreach ($order_detail_list as $key => $order_detail)
			{
				...
				if (!$order_creation_failed && isset($order->id))
				{
					...
					foreach ($order->product_list as $product)
					{
						...
						$price_pro = floatval(Product::getWholesalePrice(intval($product['id_product']), intval($product['id_product_attribute'])));
						$price_pro = Tools::ps_round($price_pro, 2);
						...
						$product_var_tpl = array(
								...
								// [ RM - START ]
								'unit_price_pro' => Tools::displayPrice($price_pro, $this->context->currency, false),
								'price_pro' => Tools::displayPrice($price_pro * $product['quantity'], $this->context->currency, false),
								// [ RM - END ]
					...
					*/
					...
					if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
					{
						...
						if (Group::getCurrent()->id == 4) {
							$total_paid = Tools::displayPrice($order->total_paid_pro, $this->context->currency, false) . " HTVA";
						} else {
						...
						if (isset($message) & !empty($message)) {
							if (Validate::isCleanHtml($message)) {
								...
								$message = "Le message ajouté à la demande a été corrompu.<br>Merci de le spécifier à nouveau lorsque nous prendrons contact avec vous pour finaliser la commande.";
							}
						} else {
							$message = "-";
						}
						...
						$data = array(
								...
								'{carrier}' => ($virtual_product || !isset($carrier->name)) ? Tools::displayError('No carrier') : ($carrier->name . ". " . $carrier->delay),
						...
						if (Validate::isEmail($this->context->customer->email))
							Mail::Send(
									(int)$order->id_lang,
									'order_conf' . (($carrier->name == "Chassart" ) ? '_no_livraison': ''),
									...
									null,
									$file_attachement,
									null, _PS_MAIL_DIR_, false, (int)$order->id_shop, 'vente@chassart.com'
							);
					}
