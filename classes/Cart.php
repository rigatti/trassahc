...
class CartCore extends ObjectModel
{
	...
	*/
	...
	public function getProducts($refresh = false, $id_product = false, $id_country = null)
	{
		...
		foreach ($result as &$row)
		{
			...
			// [[ RM - START ]]
			$row['wholesale_price'] = Product::getWholesalePrice($row['id_product'], isset($row['id_product_attribute']) ? intval($row['id_product_attribute']) : NULL);
			// [[ RM - END ]]
	...
	*/
	...
	public function getOrderTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true)
	{
		...
		foreach ($products as $product) // products refer to the cart details
		{
			...
			if ( Group::getCurrent()->id == 4 ) {
				...
				$total_price_pro = Tools::ps_round($price * (int)$product['cart_quantity'], 2);
				$order_total_pro += $total_price_pro;
			}
			//[[ RM - END ]]
		...
		$order_total_products_pro = $order_total_pro;
	...
	//[[ RM - START ]]
	function getOrderTotalWholeSale($withTaxes = false) {
		if (!$this->id)
			return 0;
		...
		foreach ($products AS $product)
		{
			//$price = floatval(Product::getWholesalePrice(intval($product['id_product']), intval($product['id_product_attribute'])));
			$price = floatval($product['wholesale_price']);
			$total_price = $price * intval($product['quantity']);
			if ($withTaxes) {
				$total_price = $total_price * (1 + ((isset($product['tax_rate']) ? $product['tax_rate'] : $product['rate']) * 0.01));
			}
			$order_total += $total_price;
		}
		$order_total_products = $order_total;
	...
	*/
	...
	public function getSummaryDetails($id_lang = null, $refresh = false)
	{
		...
		return array(
				...
				// [[ RM - START ]]
				'total_wholesale_price' => ((float)number_format($this->getOrderTotalWholeSale(), 2, '.', '')),
				// 			'total_wholesale_price_wt' => number_format($this->getOrderTotalWholeSale(true), 2, '.', ''),
				// [[ RM - END ]]
