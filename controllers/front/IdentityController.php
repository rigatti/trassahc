...
class IdentityControllerCore extends FrontController
{
	...
	public function postProcess()
	{
		...
		// [[ RM - START ]]
		if (Tools::getValue('catcode') != "") {
			$catcode = pSQL(Tools::getValue('catcode'));
			$catref = pSQL(Tools::getValue('catref'));
			$result = DB::getInstance()->getRow('SELECT COUNT(`ref`) AS total FROM `tbl_societe` WHERE tarif=\'2\' AND code=\'' . $catcode . '\' AND ref=\'' . $catref . '\'');
			if ($result['total'] > 0) {
				$this->customer->id_default_group = 4;
				$this->customer->addGroups(array(4));
				$row = array('id_default_group' => 4);
				Db::getInstance()->update('customer', $row, '`id_customer` = ' . $this->customer->id, false, true);
			}
		}
		// [[ RM - END ]]
