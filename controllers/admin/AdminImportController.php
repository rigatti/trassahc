			...
			else
			{
				...
				if (isset($info['isNew'])) {
					if ($info['isNew'] == 'OUI') {
						...
						$product->date_add = pSQL("1999-04-18 20:33:01");
					}
		...
		for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++)
		{
			...
			elseif (isset($info['image_position']) && $info['image_position'])
			{
				...
				if ($images) {
					...
					*/
					$id_image = array((int)$info['image_position']);
					// RM START
					if ($id_image) {
						//echo '$id_image:' . $id_image[0] . '-';
						//echo '$default_on:' . (int)$info['default_on'] . '-';
						//echo '$product->id:' . $product->id . '-';

						if ((int)$info['default_on'] == 1) {
							Image::updateDefaultCover($product->id, $id_image[0]);
						}
					}
					// RM END
				}
			...
			foreach (explode($this->multiple_value_separator, $info['group']) as $key => $group)
			{
				...
				$groups_attributes_label[$key][$id_attribute_group]['label'] = $type;
			}
			...
			// for each attribute .... ATTENTION MIX avec position
			...
			foreach (explode($this->multiple_value_separator, $info['attribute']) as $key => $attribute)
			{
				...
				if (isset($groups_attributes[$key]))
				{
					...
					if (!isset($attributes[$group.'_'.$attribute]) && count($groups_attributes[$key]) == 2)
					{
						...
						$obj->name[$default_language] = str_replace('\n', '', str_replace('\r', '', $groups_attributes_label[$key][$id_attribute_group]['label']));
...
//echo "setting";
						...
						if ($id_product_attribute > 0) {
							//echo 'force update READY '; 
							Db::getInstance()->execute("
								Update "._DB_PREFIX_."product_attribute_shop 
										set wholesale_price=" . (float)$info['wholesale_price'] . " where id_product_attribute=" . $id_product_attribute);
							Db::getInstance()->execute("
						...
						//echo "done : " . (float)$info['wholesale_price'];
