	...
	public function hookActionValidateOrder($params)
	{
		...
		foreach ($products as $key => $product)
		{
			if (Group::getCurrent()->id == 4) {
				$unit_price = $product['purchase_supplier_price'];
			} else {
		...
		// [ RM - START ]
		if (Group::getCurrent()->id == 4) {
			$total_paid = Tools::displayPrice($order->total_paid_pro, $currency);
		} else {
		...
		'{carrier}' => (($carrier->name == '0') ? $configuration['PS_SHOP_NAME'] : $carrier->name . " " . $carrier->delay[1]),
		...
		{
			$flagLivraison = "";
			if ($carrier->is_free == false) {
				$flagLivraison = "L ";
			}
			Mail::Send(
			$id_lang,
			'new_order',
			($flagLivraison . sprintf(Mail::l('New order : #%d', $id_lang), $order->id)),
			$template_vars,
			$merchant_mail,
			null,
