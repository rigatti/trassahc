...
function updateCartSummary(json)
{
	...
	for (i in product_list)
	{
		...
		if (priceDisplayMethod !== 0)
		{
			$('#cartProductPriceTvac_' + key_for_blockcart).html(initial_price_text);
			$('#cartProductPriceHtva_' + key_for_blockcart).html(current_price);

			$('#total_product_price_HTVA_' + key_for_blockcart).html(formatCurrency((product_list[i].wholesale_price * product_list[i].cart_quantity), currencyFormat, currencySign, currencyBlank));
			$('#total_product_price_TVAC_' + key_for_blockcart).html(formatCurrency((product_list[i].price_without_quantity_discount * product_list[i].cart_quantity), currencyFormat, currencySign, currencyBlank));
		...
		{	
			if (initial_price_text == "") {
				$('#cartProductPriceTvac_' + key_for_blockcart).html(formatCurrency((product_list[i].price_without_quantity_discount), currencyFormat, currencySign, currencyBlank));
			} else {
				$('#cartProductPriceTvac_' + key_for_blockcart).html(initial_price_text);
			}
			$('#cartProductPriceHtva_' + key_for_blockcart).html(formatCurrency((product_list[i].wholesale_price), currencyFormat, currencySign, currencyBlank));
	...
	$('#total_price_wt').html(formatCurrency(json.total_wholesale_price, currencyFormat, currencySign, currencyBlank));
	$('#total_price_without_reduction').html(formatCurrency(json.total_without_reduction_wt, currencyFormat, currencySign, currencyBlank));
