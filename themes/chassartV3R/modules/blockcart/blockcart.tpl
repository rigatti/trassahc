    ...
    <div id="cart_block" class="block exclusive">
        <div class="row">
            <div class="col-xs-12">
                <p class="titleBlock" style="float: left; height: 50px; padding: 15px 15px; font-size: 18px; line-height: 20px;">
                    <a href="{$link->getPageLink("$order_process", true)|escape:'html'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">{l s='Cart' mod='blockcart'}{if $ajax_allowed}<span id="block_cart_expand" {if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !isset($colapseExpandStatus)}class="hidden"{/if}>&nbsp;</span><span id="block_cart_collapse" {if isset($colapseExpandStatus) && $colapseExpandStatus eq 'collapsed'}class="hidden"{/if}></span>{/if}</a>
	            </p>
	        </div>
	    </div>
	    <div class="block_content">
            ...
            {if $products}
                ...
                {foreach from=$products item='product' name='myLoop'}
                    ...
                    <dt id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
                        ...
                        <span class="price">
                            {if $isProfesional}
                                {displayWtPrice|html_entity_decode:2:'UTF-8' p=($product.wholesale_price * $product.cart_quantity|intval)}
                            {else}
            ...
            {/if}
                ...
                <p id="cart-prices">
                    <span id="cart_block_total" class="price ajax_block_cart_total">{if $isProfessional}{$total_pro}{else}{$total}{/if}</span>
                    <span>{l s='Total' mod='blockcart'} {if $isProfessional} HTVA{/if}</span>
                </p>
                <p id="cart-buttons">
                    ...
                    <a href="{$link->getPageLink("$order_process", true)|escape:'html'}" id="button_order_cart" class="btn btn-success btn-sm" title="{l s='Check out' mod='blockcart'}" rel="nofollow">D&eacute;tail et commande</a>
                </p>
            </div>
	    </div>
    </div>
