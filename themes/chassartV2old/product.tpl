...
var isProfessional = {if $isProfessional}true{else}false{/if};
// Customizable field
		...
		{if ($product->show_price AND !isset($restricted_country_mode)) OR isset($groups) OR $product->reference OR (isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS)}
			...
			<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html'}" method="post">
				...
				<div class="product_attributes">
						...
						<div id="attributes">
							...
							{foreach from=$groups key=id_attribute_group item=group}
								{if $group.attributes|@count}
									...
									<label class="attribute_label">S&eacute;lection :</label><br>
									<div id="currentSelection" class="padding-left-small">
										<script type="text/javascript">
											<!--
												$( document ).ready(function() {
													$("#currentSelection").html($("#group_{$id_attribute_group|intval} :selected").text() + "<br>");
													document.getElementById('group_{$id_attribute_group|intval}').size=7;
													document.getElementById('group_{$id_attribute_group|intval}').focus();
												});
											//-->
										</script>
									</div>
					...
					{if $isProfessional}
						<p>
							<label>{l s='Prix Professionnel :'}</label><br>
							<span id="our_price_display" class="padding-left-small display_for_pro_span">{convertPrice price=$productPrice}</span> <span class="display_for_pro_span">HTVA</span> 
						</p>
						<p>
							<label>{l s='Prix Particulier :'}</label><br>
							<span id="our_price_display_alternate" class="padding-left-small display_for_pro_span">{convertPrice price=$productPriceWithoutReduction}</span> <span class="display_for_pro_span">TVAC</span> 
						</p>
					{else}
						<p>
							<label>{l s='Prix :'}</label><br>
							<span id="our_price_display" class="padding-left-small display_for_pro_span">{convertPrice price=$productPrice}</span> <span class="display_for_pro_span">TVAC</span> 
							<span id="id="our_price_display_alternate" style="display:none;">
						</p>
					{/if}
					<br>
					<!-- quantity wanted -->
					<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
						...
						<span class="padding-left-small"><input type="text" name="qty" id="quantity_wanted" class="text" value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}" size="2" maxlength="3" {if $product->minimal_quantity > 1}onkeyup="checkMinimalQuantity({$product->minimal_quantity});"{/if} /></span>
					</p>
				...
				{else}
					<div class="content_prices content_prices_upper clearfix">
				{/if}
						...
						<div id="attributes">
							...
							{foreach from=$groups key=id_attribute_group item=group}
								{if $group.attributes|@count}
									{assign var="groupName" value="group_$id_attribute_group"}
									<fieldset>
										<label class="attribute_label" for="group_{$id_attribute_group|intval}">Conditionnements disponibles :&nbsp;</label><br>
										<div class="attribute_list testpadding-left-small">
											<select name="{$groupName}" id="group_{$id_attribute_group|intval}" class="attribute_select" onchange="findCombination();getProductAttribute();document.getElementById('currentSelection').innerHTML=this.options[this.selectedIndex].text + '<br>'">
...
<div id="more_info_block" class="clear">
	<ul id="more_info_tabs" class="idTabs idTabsShort clearfix">
		{if $product->description}<li><a id="more_info_tab_more_info" href="#idTab1">DESCRIPTION</a></li>{/if}
