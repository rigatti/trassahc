...
{if !$opc}
	...
	<!--form id="form" action="/p156/index.php?fc=module&module=cashondelivery&controller=validation&id_lang=1" method="post" onsubmit="return acceptCGV();"-->
	<form action="{$link->getModuleLink('cashondelivery', 'validation', [], true)|escape:'html'}" method="post" onsubmit="return acceptCGV();">
		<input type="hidden" name="confirm" value="1" />
		<p>
		Vous serez tr&egrave;s prochainement recontact&eacute; par nos services afin d'obtenir un bon de commande complet reprennant les cautions, frais de livraison et la disponibilit&eacute; des produits. 
		<br>
	</p>
					...
					<label for="delivery_option_{$id_address}_{$option@index}">
						<table class="resume">
							<tr>
								...
								<br>
								{if $key == 6}
									<b><a href="/ps1609/content/1-conditions-livraison" target="_blank">Lien vers nos conditions de livraison</a></b>
								{/if}
								</td>
								<td>
								</td>
...
{if $conditions AND $cms_id}
	<h3 class="condition_title"><label for="cgv">{l s='Terms of service'}</label></h3>
	<p class="checkbox">
		<input type="checkbox" name="cgv" id="cgv" value="1" {if $checkedTOS}checked="checked"{/if} autocomplete="off"/>
		<label for="cgv">{l s='I agree to the terms of service and will adhere to them unconditionally.'}</label> <a href="{$link_conditions}" target="_blank">{l s='(Read the Terms of Service)'}</a>
	</p>
	<script type="text/javascript">
		$(document).ready(function() {
	        $("a.iframe").fancybox({
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
			<input type="submit" name="processCarrier" value="{l s='Je confirme ma demande'} &raquo;" class="exclusive" />
		{/if}
	</p>
</form>
{else}
