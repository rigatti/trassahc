	...
	{foreach from=$products item=product name=products}
		<li class="ajax_block_product {if $smarty.foreach.products.first}first_item{elseif $smarty.foreach.products.last}last_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if} clearfix">
			...
			<div class="center_block">
				...
				<h3>{if isset($product.pack_quantity) && $product.pack_quantity}{$product.pack_quantity|intval|cat:' x '}{/if}<a href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|truncate:120:'...'|escape:'htmlall':'UTF-8'}</a></h3>
			...
			<div class="right_block">
				...
				<span>{$product.name_default_product_attribute}<br></span>
				<!--div class="content_price">
					{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode) && $product.price != 0}
						{if $isProfessional}
							<span class="price" style="display: inline;">{convertPrice price=$product.wholesale_price} HTVA</span><br>
						{else}
							<span class="price" style="display: inline;">{convertPrice price=$product.price}</span><br>
						{/if}
					{/if}
				</div-->
				...
				{/if}
				-->
				<a class="button lnk_view" href="{$product.link|escape:'htmlall':'UTF-8'}" title="{l s='View'}">Description du produit<br>Conditionnements<br>Prix</a>
			</div>
		</li>
	{/foreach}
	</ul>
